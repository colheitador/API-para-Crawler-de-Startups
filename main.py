...
def pesquisar_por_cidade_ou_sigla_do_estado(cidade_ou_estado: str):
    # para isso, vamos criar um filtro para que o usuário busque por uma palavra ou sequencia de caracteres presentes no nome da cidade ou na sigla do estado da empresa
	...
	return numero_de_resultados, resultados # caso queira mostrar os resultados da busca em .json, comente esta linha e descomente a "return json.dumps(resultados)"
...
@app.get("/startups/descricao")
def pesquisar_por_descricao(descricao: str):
    # para isso, vamos criar um filtro para que o usuário busque por uma palavra ou sequência de caracteres na empresa	
	selecionar_palavra_buscada = f"SELECT * FROM tab_startups_3 WHERE sobre_c LIKE ('%{descricao}%') "
	cursor3.execute(selecionar_palavra_buscada)
	informacoes_sobre_startups = cursor3.fetchall()
	resultados = [] ## os resultados (em forma de dicionarios) serao adicionados a essa lista
	for cada_linha_tabela_de_startups in informacoes_sobre_startups:
		cada_dicionario_de_startups = {
        ...
        #print(cada_linha_tabela_de_startups)
        #resultados.append(cada_linha_tabela_de_startups)
	numero_de_resultados = "Foram encontrados " + str(len(resultados)) + " resultados para esta busca: " + descricao

	return numero_de_resultados, resultados # caso queira mostrar os resultados da busca em .json, comente esta linha e descomente a "return json.dumps(resultados)"
	#return numero_de_resultados, json.dumps(resultados) # caso Não queira mostrar os resultados da busca em .json, comente esta linha e  descomente a "return resultados"


@app.get("/startups/nome")
def pesquisar_por_nome(nome: str):
    # para isso, vamos criar um filtro para que o usuário busque por uma palavra ou sequencia de caracteres presentes no nome empresa	
	selecionar_palavra_buscada = f"SELECT * FROM tab_startups_3 WHERE nome_da_startup_c LIKE ('%{nome}%')"
	cursor3.execute(selecionar_palavra_buscada)
	informacoes_sobre_startups = cursor3.fetchall()
	resultados = [] ## os resultados (em forma de dicionarios) serao adicionados a essa lista
	for cada_linha_tabela_de_startups in informacoes_sobre_startups:
        ...
        #print(cada_linha_tabela_de_startups)
        #resultados.append(cada_linha_tabela_de_startups)
	numero_de_resultados = "Foram encontrados " + str(len(resultados)) + " resultados para esta busca: " + nome
